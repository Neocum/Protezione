...
const neo1 = require("discord.js");
const neo2 = require("discord.js");
const neo3 = require("discord.js");

const client1 = new neo1.Client();
const client2 = new neo2.Client();
const client3 = new neo3.Client();

const ayarlar = require('./ayarlar.json');
const config = require('./neo.json');
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Néo Guard | İzinsiz kullanıcı yasaklama"}).catch(e => { })	

  let channel = client1.channels.cache.get(ayarlar.defenderlog)
   if (!channel) return console.log('Ban Koruma Logu Yok!');

   const neo = new Discord.MessageEmbed()
...
.setDescription(`${yetkili.user} yetkilisi bir kullanıcıyı izinsiz yasakladığı için yetkiliyi yasakladım.\n─────────────────────\nYetkili: (${yetkili.user} - \`${yetkili.id}\`)\nKullanıcı: \`${user.tag}\` - \`${user.id}\`\n─────────────────────\nTarih: \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\``)
  channel.send(`@here`, {embed: neo}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Néo Guard | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  member.guild.roles.cache.forEach(async function(neo) {
  if (neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});

  member.guild.members.ban(entry.executor.id, {reason: "Néo Guard | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Néo Guard | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.roles.cache.forEach(async function(neo) {
  if (neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});

  newGuild.members.ban(entry.executor.id, {reason: "Néo Guard | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Néo Guard | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.roles.cache.forEach(async function(neo) {
  if(neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});  

  role.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Néo Guard | Rol Koruma Sistemi" });
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.roles.cache.forEach(async function(neo) {
  if(neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});  

  newRole.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client2.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  newMember.roles.set(oldMember.roles.cache.map(r => r.id));
    uyecik.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz Yönetici Verme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.roles.cache.forEach(async function(neo) {
  if(neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});

  channel.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Néo Guard | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@here`, {embed: neo}).catch(e => { })
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Néo Guard | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.cache.roles.cache.forEach(async function(neo) {
  if(neo.permissions.has("ADMINISTRATOR") || neo.permissions.has("BAN_MEMBERS") || neo.permissions.has("MANAGE_GUILD") || neo.permissions.has("KICK_MEMBERS") || neo.permissions.has("MANAGE_ROLES") || neo.permissions.has("MANAGE_CHANNELS")) {
    neo.setPermissions(0).catch(err =>{});}});

  newChannel.guild.members.ban(entry.executor.id, { reason: `Néo Guard | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Néo Guard | Webhook Silindi!"}))
  channel.guild.members.ban(entry.executor.id, {reason: "Néo Guard | İzinsiz Webhook Açmak!"})
...
.setDescription(`${entry.executor} üyesi izinsiz emoji sildi ve kullanıcıyı karantina atıp, emojiyi yeniden yükledim.\n─────────────────────\nYetkili: (${entry.executor} - \`${entry.executor.id}\`)\nEmoji: \`${emoji.name}\` - \`${emoji.id}\`\n─────────────────────\nTarih: \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\``)
  channel.send(`@here`, {embed: neo}).catch(err => { })
return;
});
